...
/* import { fetchAllAgents } from "../../../utils/fetchData"; */
import FormNewAgent from "../../FormNewAgent";
import Portal from "../../Portal/Portal";
import CardBase from "../CardBase";
import Item from "./Item";
import { Container } from "./styles";

export default function AgentConfig({ all }) {
  const {
    showAgentConfig,
    setShowAgentConfig,
    actionDone,
  ...
  const [groups, setAgents] = useState([]);

  const handlePortalAgent = () => {
    setIsOpenAgent(true);
  };
  ...
  return (
    <CardBase isShown={showAgentConfig} setIsShown={setShowAgentConfig}>
      <Portal absolute="true" isOpen={isOpenAgent} setIsOpen={setIsOpenAgent}>
        <label>Adicionar novo agente</label>
        <FormNewAgent />
      </Portal>
      <Container>
        <div className="config-task-top">
          <div className="label">
            ...
            <span>Configurações de agente</span>
          </div>
          <svg
            width="15"
            height="15"
