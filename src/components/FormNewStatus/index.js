...
import { addNewStatus } from "../../utils/persistData";
import { ButtonContainer } from "../Buttons/save";
import ColorPicker from 'material-ui-color-picker'

export default function FormNewStatus() {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object().shape({
      ...
      color: yup.string().defined("Este campo é obrigatório"),
    }),
    onSubmit: (
      {
        name,
        ...
        .catch(({ response }) => {
          ...
          if (response.status === 422) {
            ...
            if (response.data.errors.color) {
              setErrors({
                color: `${response.data.errors.color[0]}`,
              });
            }
          } else {
            Swal.fire({
              text: `${
                response.mensagem
                  ? response.mensagem
                  : "Erro ao adicionar um novo estado para as tarefas"
              }`,
              icon: "error",
    ...
    <Container onSubmit={formik.handleSubmit}>
        <div className="first form-control">
          <div className="label-control">
            <label htmlFor="client">Nome do estado*</label>
          </div>
          <input
        ...
        <div className="first form-control">
          <div className="label-control">
            <label htmlFor="client">Escolha uma cor para identificar o estado*</label>
          </div>

          <ColorPicker
            className={formik.errors.color ? "client red-border" : "client "}
            id="color"
            name='color'
            type="color"
