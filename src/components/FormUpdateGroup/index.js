...
import { updateGroup } from "./../../utils/persistData";

export default function FormUpdateGroup({group}) {
  const { enqueueSnackbar } = useSnackbar();
  const { actionDone, setActionDone, setIsOpenGroup, setRefresh, refresh } = useGlobal();
          ...
          if (response.status === 422) {
            ...
            Swal.fire({
              text: `${
                response.mensagem
                  ? response.mensagem
                  : "Erro ao editar o grupo de utilizador"
              }`,
              icon: "error",
    ...
    <Container onSubmit={formik.handleSubmit}>
        <div className="first form-control">
          <div className="label-control">
            <label htmlFor="client">Nome do grupo*</label>
          </div>
          <input
        ...
        >
          <span> {formik.isSubmitting ? "a editar..." : "Guardar alterações"} </span>
