...
import { updateType } from "./../../utils/persistData";

export default function FormUpdateType({ type }) {
          ...
          if (response.status === 422) {
            ...
            Swal.fire({
              text: `${
                response.mensagem
                  ? response.mensagem
                  : "Erro ao editar o tipo de tarefa"
              }`,
              icon: "error",
    ...
    <Container onSubmit={formik.handleSubmit}>
        <div className="first form-control">
          <div className="label-control">
            <label htmlFor="client">Tipo de tarefa*</label>
          </div>
          <input
